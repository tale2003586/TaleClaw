"""Deterministic extraction and transactional semantic compaction for TaskState."""
...
def _invoke_semantic(callback: Callable[..., Any], state: TaskState, events: list[dict[str, Any]]) -> Any:
    """Allow both callback(state, events) and callback(*, state, events)."""
    try:
        signature = inspect.signature(callback)
    except (TypeError, ValueError):
        return callback(state=state, events=events)
    parameters = signature.parameters.values()
    accepts_keywords = any(parameter.kind == parameter.VAR_KEYWORD for parameter in parameters)
    names = signature.parameters
    if accepts_keywords or {"state", "events"}.issubset(names):
        return callback(state=state, events=events)
...
class StatePatch:
    ...
    def from_payload(cls, payload: Any) -> "StatePatch":
        ...
        return cls(
            ...
            tool_call_fingerprints=[str(value) for value in payload.get("tool_call_fingerprints") or []],
            failed_strategies=[dict(value) for value in payload.get("failed_strategies") or [] if isinstance(value, dict)],
            non_retryable_failures=[dict(value) for value in payload.get("non_retryable_failures") or [] if isinstance(value, dict)],
            result_hashes=[str(value) for value in payload.get("result_hashes") or []],
            observed_effects=[dict(value) for value in payload.get("observed_effects") or [] if isinstance(value, dict)],
    ...
    def extract(self, events: Iterable[dict[str, Any]]) -> StatePatch:
        ...
        for event in events:
            ...
            if event_type == "tool_result":
                ...
                result_hash = evidence.content_hash or hashlib.sha256(
                    json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str).encode("utf-8")
                ).hexdigest()
                patch.result_hashes.append(result_hash)
                patch.observed_effects.append({
                    "event_id": event_id,
                ...
                if status in {"error", "failed", "denied"}:
                    failure = {"event_id": event_id, "summary": evidence.summary, "status": status}
                    patch.failed_strategies.append(failure)
                    if status == "denied" or payload.get("non_retryable"):
                        patch.non_retryable_failures.append(failure)
                ...
                for call in _tool_calls(payload):
                    ...
                    encoded = json.dumps(call, ensure_ascii=False, sort_keys=True, default=str)
                    patch.tool_call_fingerprints.append(
...
class SemanticCompactor:
    """Injectable semantic patch producer; it cannot mutate or replace TaskState."""

    def __init__(
        self,
        callback: Callable[..., StatePatch | dict[str, Any] | None] | None = None,
        *,
        provider: Any | None = None,
        model: str = "",
        max_tokens: int = 2400,
    ):
        if callback is not None and provider is not None:
            raise ValueError("provide either callback or provider, not both")
        self._callback = callback
        self._provider = provider
        self._model = str(model or "")
        self._max_tokens = max(256, int(max_tokens))

    def propose(self, *, state: TaskState, events: Sequence[dict[str, Any]]) -> StatePatch:
        if self._callback is None and self._provider is None:
            return StatePatch(
                source_event_ids=[_event_id(event) for event in events if _event_id(event)],
                origin="semantic",
            )
        callback = self._callback
        if callback is None:
            callback = getattr(self._provider, "propose_state_patch", None) or getattr(self._provider, "compact", None)
        if callable(callback):
            proposal = _invoke_semantic(callback, deepcopy(state), list(events))
        elif callable(getattr(self._provider, "chat", None)):
            proposal = self._propose_with_chat(state=state, events=events)
        else:
            raise CompactionError("semantic provider has no patch or chat interface")
        if proposal is None:
        ...
        unsupported = [finding for finding in patch.findings if not finding.evidence_refs]
        patch.findings = [finding for finding in patch.findings if finding.evidence_refs]
        patch.hypotheses.extend(
            Hypothesis(
                id=finding.id,
                claim=finding.claim,
                rationale="semantic claim had no EvidenceRef and was downgraded",
    ...
    *,
    ...
    if patch.origin == "semantic":
        ...
        deterministic_fields = [
            patch.tool_call_fingerprints,
            patch.failed_strategies,
            patch.non_retryable_failures,
            patch.result_hashes,
            patch.observed_effects,
...
def _apply_patch(state: TaskState, patch: StatePatch, *, validate: bool) -> TaskState:
    ...
    memory = next_state.execution_memory
    memory.tool_call_fingerprints = list(dict.fromkeys([
        *memory.tool_call_fingerprints, *patch.tool_call_fingerprints,
    ]))[-200:]
    memory.failed_strategies = [*memory.failed_strategies, *deepcopy(patch.failed_strategies)][-100:]
    memory.non_retryable_failures = [
        *memory.non_retryable_failures, *deepcopy(patch.non_retryable_failures)
    ][-100:]
    memory.result_hashes = list(dict.fromkeys([*memory.result_hashes, *patch.result_hashes]))[-200:]
    memory.observed_effects = [*memory.observed_effects, *deepcopy(patch.observed_effects)][-200:]
...
class ContextCheckpoint:
    ...
    @property
    def source_event_start_id(self) -> str:
        return self.source_event_ids[0] if self.source_event_ids else ""

    @property
    def source_event_end_id(self) -> str:
        return self.source_event_ids[-1] if self.source_event_ids else ""

    @classmethod
    def create(cls, *, generation: int, state: TaskState, source_event_ids: list[str], boundary: str) -> "ContextCheckpoint":
        return cls(
            generation=generation, state_version=state.version,
            source_event_ids=list(source_event_ids), compacted_until_event_id=boundary,
            artifact_refs=list(state.artifact_refs),
            checksum=payload_checksum(state.to_dict()), created_at=_now(),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "compacted_until_event_id": self.compacted_until_event_id,
            "artifact_refs": self.artifact_refs, "checksum": self.checksum,
...
class CompactionResult:
    ...
    compacted_until_event_id: str | None
    archived_event_ids: list[str]
    patches: list[StatePatch]


class CompactionCoordinator:
    """Coordinates all persistence callbacks before exposing a new boundary.

    Callbacks may persist to a SessionStore transaction.  The incoming state is
    never mutated, so failures leave its state and the caller's boundary intact.
    """

    def __init__(
        self,
        *,
        checkpoint_writer: Callable[[ContextCheckpoint], Any],
    ...
    def compact(
        ...
        *,
        ...
        for _attempt in range(2):
            try:
                semantic = self.semantic_compactor.propose(state=candidate, events=eligible)
                candidate = reduce_task_state(candidate, semantic, max_tokens=max_tokens)
                semantic_error = None
                break
            except Exception as exc:
                semantic_error = exc
        if semantic_error is not None or semantic is None:
            raise CompactionError("semantic StatePatch failed after one retry") from semantic_error
        candidate.execution_memory.compaction_generation = state.execution_memory.compaction_generation + 1
        boundary = _event_id(eligible[-1])
        checkpoint = ContextCheckpoint.create(
            generation=candidate.execution_memory.compaction_generation,
            state=candidate, source_event_ids=[_event_id(event) for event in eligible], boundary=boundary,
        )
        completion = {
            "event_type": "compaction_completed", "event_id": f"compaction:{checkpoint.generation}:{boundary}",
            "payload": {"generation": checkpoint.generation, "checkpoint_checksum": checkpoint.checksum,
                        "compacted_until_event_id": boundary},
        }
        try:
            manager = self.transaction_factory() if self.transaction_factory else nullcontext()
            with manager:
                self.checkpoint_writer(checkpoint)
                self.completion_writer(completion)
