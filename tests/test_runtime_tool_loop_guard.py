...
def _pipeline(registry, provider, *, hooks=None, max_reasoning_steps=24) -> Runtime:
    return Runtime(
        tools=registry,
        provider=provider,
        model="test-model",
...
class RuntimeToolLoopGuardTests(unittest.TestCase):
    ...
    def test_pipeline_reuses_context_prefix_across_reasoning_steps(self) -> None:
        registry = _registry_with_tool("read_file", allowed_agents={"coding"}, always_on=True)
        provider = ScriptedProvider([
    ...
    def test_repeated_invisible_tool_request_stops_turn(self) -> None:
        registry = _registry_with_tool("git_add", allowed_agents={"coding"})
    ...
    def test_pipeline_includes_tool_catalog_in_model_context(self) -> None:
        ...
        pipeline = _pipeline(registry, provider)
        session = Session(id="web:test", active_agent="bot")

        reply = pipeline.run_turn(session, SimpleNamespace(tool_mode="bot"))
    ...
    def test_tool_loop_history_resets_between_user_turns(self) -> None:
        ...
        first = pipeline.run_turn(session, SimpleNamespace(tool_mode="bot"))
        second = pipeline.run_turn(session, SimpleNamespace(tool_mode="bot"))
