...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => bottom.current?.scrollIntoView?.({ behavior: stream.status === "streaming" || stream.status === "finalizing" ? "auto" : "smooth", block: "end" }));
  ...
  const send = async () => {
    ...
    if (!message || stream.status === "streaming" || stream.status === "finalizing" || attachmentBusy || raw) return;
    shouldStickToBottom.current = true;
    setDraft("");
    setAttachments([]);
    setAttachmentError("");
  ...
  const changeMode = async (command: string, label: string) => {
    if (raw || stream.status === "streaming" || stream.status === "finalizing") return;
    setModeBusy(command);
    setModeNotice("");
    try {
  ...
  return <div className="page chat-page">
    ...
    <div className="message-scroll" ref={messageScroller} onScroll={handleMessageScroll}>
      <div className="message-list" ref={messageList}>
        ...
        {(stream.status === "streaming" || stream.status === "finalizing" || stream.status === "stopping" || stream.text || stream.thinking || stream.error) && <article className={`message assistant streaming ${stream.error ? "error" : ""}`}><span className="message-role">Agent</span><div><ActivityTimeline items={stream.activity} status={stream.status === "error" ? "error" : stream.status === "finalizing" ? "complete" : "running"} startedAt={stream.startedAt} finishedAt={stream.finishedAt} />{stream.progressText && <p className="attachment-progress">{stream.progressText}</p>}{stream.thinking && <ThinkingContent content={stream.thinking} open />}{stream.text && <SafeMarkdown>{stream.text}</SafeMarkdown>}{stream.error && <p className="inline-error">{stream.error}</p>}</div></article>}
        <div ref={bottom} />
      </div>
    </div>
    <div className="composer-dock"><div className="composer">{attachments.length > 0 && <div className="attachment-list">{attachments.map((file, index) => <span className="attachment-chip" key={`${file.name}-${file.size}-${index}`}><Paperclip aria-hidden="true" size={13} />{file.name}<button type="button" aria-label={`移除 ${file.name}`} onClick={() => setAttachments((current) => current.filter((_, itemIndex) => itemIndex !== index))}><X aria-hidden="true" size={12} /></button></span>)}</div>}{attachmentError && <p className="composer-error">{attachmentError}</p>}<textarea ref={composerInput} value={draft} disabled={raw || stream.status === "streaming" || stream.status === "finalizing" || attachmentBusy} onChange={(event) => { setDraft(event.target.value); event.currentTarget.style.height = "auto"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 190)}px`; }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} placeholder={raw ? "只读会话" : "输入问题，并可添加 PDF、Word、PPT 或图片附件……"} /><div><span>{attachmentBusy ? "正在上传附件" : stream.status === "streaming" ? "思考与执行中" : stream.status === "finalizing" ? "回答已完成，正在保存" : stream.status === "stopping" ? "正在停止" : raw ? "只读会话" : "附件将先经 MinerU 精准解析"}</span>{stream.status === "streaming" ? <Button onClick={() => void stream.stop(sessions.activeId)}><CircleStop aria-hidden="true" size={14} />停止</Button> : <div className="composer-actions"><label className="model-picker"><span>模型</span><select value={modelProfile} disabled={raw || attachmentBusy || stream.status === "finalizing"} onChange={(event) => { setModelProfile(event.target.value); setThinkingEnabled(false); }} aria-label="选择模型">{models.length === 0 && <option value="">自动路由</option>}{models.length > 0 && <option value="">自动路由</option>}{models.map((option: ModelOption) => <option key={option.profile} value={option.profile}>{option.model} · {option.provider}</option>)}</select></label><Button title={thinkingAvailable ? "启用模型深度思考" : "当前模型不支持思考模式"} disabled={!thinkingAvailable || raw || attachmentBusy || stream.status === "finalizing"} className={thinkingEnabled ? "active" : ""} onClick={() => setThinkingEnabled((value) => !value)}>深度思考</Button><Button aria-label="添加附件" disabled={raw || attachmentBusy || stream.status === "finalizing"} onClick={() => attachmentInput.current?.click()}><Paperclip aria-hidden="true" size={14} />附件</Button><input ref={attachmentInput} hidden multiple type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.png,.jpg,.jpeg,.jp2,.webp,.gif,.bmp" onChange={(event) => selectAttachments(event.target.files)} /><Button className="primary" onClick={() => void send()} disabled={(!draft.trim() && !attachments.length) || raw || attachmentBusy || stream.status === "finalizing"}><Send aria-hidden="true" size={14} /><span>发送</span></Button></div>}</div></div></div>
  </div>;
}
