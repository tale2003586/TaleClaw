...
import { Button, EmptyState, ErrorState, JsonDisclosure, PageHeader, Skeleton } from "../components/ui";
import { formatTime } from "../components/trace/TraceEventView";

export default function LogsPage() {
  const { runs, activeRunId, activeDetail, loadRuns, selectRun } = useRuns();
  ...
  return <div className="page logs-page"><PageHeader eyebrow="Observability" title="系统日志" description="查看每次 Agent Run 的结构化 Trace 事件。" action={<Button className="primary" onClick={() => void refresh()}><RefreshCw aria-hidden="true" size={14} />刷新日志</Button>} />
    <div className="page-body logs-body"><div className="metric-strip"><Metric label="总计" value={all.length} /><Metric label="错误" value={all.filter((row) => row.level === "error").length} tone="error" /><Metric label="警告" value={all.filter((row) => row.level === "warn").length} tone="warn" /></div>
      ...
      {runs.status === "success" && (runs.data?.length || 0) === 0 && <EmptyState title="还没有运行日志" message="发起一次聊天任务后，Run 的结构化 Trace 会显示在这里。" />}
